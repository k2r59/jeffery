...
final class CoachSession: ObservableObject {
    ...
    private func sendGreeting() {
        ...
        if goal.kind == .free {
            ...
            realtime.requestResponse(instructions: "Salue-le par son prénom en une phrase chaleureuse, sans dire ton nom ni te présenter : il sait que c'est toi.\(name) Puis une seule question courte : « Tu veux quoi aujourd'hui ? » Il peut répondre un temps (« 30 minutes tranquille »), une distance (« 5 km »), « à ma façon » ou « propose-moi un truc ». Réponse incompréhensible ou ambiguë : redemande en une question courte, ne devine pas. Temps ou distance : set_goal tout de suite, sans redemander. À sa façon : set_goal free et tu accompagnes. Proposition, exercice, fractionné, programme : suggest_workouts (jamais un programme de tête), deux options numérotées en une phrase chacune ; il choisit, tu reformules son choix et demandes « c'est bien ça ? », puis start_workout(confirmed=true) à son oui. Une seule question au départ, ensuite on part.")
        } else {
